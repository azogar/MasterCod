...
public class MultiConnector implements Connector {
	...
	public final static Logger LOGGER = Logger.getLogger(UDPConnector.class
			.toString());
	...
	public MultiConnector(InetSocketAddress address) {
	...
	public synchronized void start() throws IOException {
		...
		LOGGER.config("UDP-connector starts " + senderCount
				+ " sender threads and " + receiverCount + " receiver threads");

		receiverThreads = new LinkedList<Thread>();
		for (int i = 0; i < receiverCount; i++) {
			receiverThreads.add(new Receiver("UDP-Receiver " + localAddr + "["
					+ i + "]"));
		}
		...
		System.out.println("UDP connector listening on "
				+ socket.getLocalSocketAddress() + ", recv buf = "
				+ receiveBuffer + ", send buf = " + sendBuffer
				+ ", recv packet size = " + receiverPacketSize
	...
	private class Receiver extends Worker {
		...
		protected void work() throws IOException {
			...
			if (logPackets)
			LOGGER.info("Connector (" + socket.getLocalSocketAddress()
						+ ") received " + datagram.getLength() + " bytes from "
						+ datagram.getAddress() + ":" + datagram.getPort());

			byte[] bytes = Arrays.copyOfRange(datagram.getData(),
	...
	private class Sender extends Worker {
		...
		protected void work() throws InterruptedException, IOException {
			...
			if (logPackets)
				LOGGER.fine("Connector (" + socket.getLocalSocketAddress()
						+ ") sends " + datagram.getLength() + " bytes to "
