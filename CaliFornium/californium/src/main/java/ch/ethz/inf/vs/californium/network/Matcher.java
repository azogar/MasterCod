...
public class Matcher {
	...
	public void sendRequest(Exchange exchange, Request request) {
		...
		//XXX che l'accrocchio sia con me!	
		KeyToken idByTok;
		if (request.isMulticast()) {
			idByTok = new KeyToken(request.getToken(),
					new byte[0], request.getDestinationPort());
...
//XXX che l'accrocchio sia con me!		
		if (exchange == null) {
			idByTok = new KeyToken(response.getToken(),
						new byte[0], response.getSourcePort());
			exchange = exchangesByToken.get(idByTok);
		}
